...
import { ShoppingCart, Star, Check, X, Plus, Minus, Zap, Shield, Clock, User, History } from 'lucide-react'
import LoginModal from './components/LoginModal'
import PurchaseHistoryModal from './components/PurchaseHistoryModal'

const SimbioIA = () => {
  // Usuário logado (null se não estiver logado)
  const [user, setUser] = useState(null)
  // Controle de abertura do modal de login
  ...
  const [purchaseHistory, setPurchaseHistory] = useState(() => {
    // Carrega histórico salvo no localStorage se existir (apenas no navegador)
  ...
  // Handler de login (recebe objeto userData e atualiza estado)
  ...
  // Finaliza a compra: grava o pedido no histórico e limpa o carrinho
  const finalizePurchase = () => {
    if (cart.length === 0) return
    // Se não estiver logado, solicita login antes de finalizar
    if (!user) {
    ...
    const updatedHistory = [...purchaseHistory, newOrder]
            ...
            <div className="flex items-center space-x-4">
              {user ? (
                ...
                <button
                  ...
                  className="bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 flex items-center space-x-2 shadow-lg"
                >
              ...
              {user && (
                <button
                  ...
                  className="bg-gradient-to-r from-blue-500 to-cyan-500 hover:from-blue-600 hover:to-cyan-600 px-6 py-3 rounded-xl font-semibold text-white transition-all duration-200 flex items-center space-x-2 shadow-lg"
                >
              ...
              {cart.length > 0 && (
                <div className="border-t border-white/20 p-6 space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-600 hover:to-emerald-600 py-4 rounded-xl font-bold text-white transition-all duration-200 shadow-lg"
                  >
                    Finalizar Compra
                  </button>
